...
export default function Library() {
  ...
  const { user, setQueue } = usePlayerStore()
  ...
  const loadLibrary = async () => {
    ...
    setLoading(true)
    const { data, error } = await supabase.from('library').select('*').eq('user_phone', user.phone).order('added_at', { ascending: false })
    if (!error) setSongs(data || [])
    setLoading(false)
  }

  useEffect(() => { loadLibrary() }, [user?.phone])

  const handleDelete = async (videoId) => {
    if (!user?.phone) return
    const { error } = await supabase.from('library').delete().match({ video_id: videoId, user_phone: user.phone })
