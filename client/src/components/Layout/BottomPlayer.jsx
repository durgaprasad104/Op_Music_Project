...
export default function BottomPlayer() {
  ...
  const { seekTo, togglePlay, playNextTrack, playPrevTrack } = usePlayerContext() || {}
  const {
    user, currentTrack, isPlaying, volume, isMuted, progress, duration,
    setIsPlaying, setVolume, setMuted, setDetailTrack
  } = usePlayerStore()
  ...
  const handleSave = async () => {
    if (!currentTrack || !user?.phone) return
    try {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        {/* Track info */}
        <div 
          style={{ display: 'flex', alignItems: 'center', gap: 12, minWidth: 0, flex: '1 1 200px', maxWidth: 280, cursor: 'pointer' }}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
          ...
          <button
            onClick={() => {
              if (duration && progress) seekTo?.(Math.max(0, progress - 10))
            }}
            style={{
          ...
          <button
            onClick={() => {
              if (duration && progress) seekTo?.(Math.min(duration, progress + 10))
            }}
            style={{
              background: 'none', border: 'none', cursor: 'pointer', padding: 6, borderRadius: '50%',
              color: 'rgba(148,163,184,0.7)', transition: 'color 0.2s, background 0.2s',
