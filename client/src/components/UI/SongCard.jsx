...
export default function SongCard({ track, queue = [], queueIndex = 0, onAddToPlaylist }) {
  ...
  const handlePlay = () => {
    // Call playTrack synchronously inside the click handler so the YouTube API
    // call happens within the browser's user-gesture context (required for audio).
    if (playTrack) {
      if (queue.length > 0) playTrack(track, queue, queueIndex)
      else playTrack(track)
    } else {
      // Fallback before context is ready
